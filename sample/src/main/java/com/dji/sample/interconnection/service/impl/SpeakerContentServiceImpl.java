...
import static com.dji.sample.interconnection.service.impl.SpeakerJobServiceImpl.AUDIO_FILE_PREFIX;
        ...
        switch (param.getType()) {
            case AUDIO:
                if (Objects.isNull(file)) {
                    throw new RuntimeException("类型为pcm音频时，file文件不能为空");
                }
            ...
            case TTS:
                ...
                if (!StringUtils.hasText(param.getName())) {
                    contentDTO.name("TTS-" + DateUtil.format(new Date(), DatePattern.PURE_DATETIME_PATTERN));
                }
