...
public class HuaweiObsServiceImpl implements IOssService {
    ...
    public CredentialsToken getCredentials() {
        try {
            ...
            return Objects.isNull(securityKeyBean) ? null :
                    new CredentialsToken(securityKeyBean.getAccessKey(), securityKeyBean.getSecretKey(),
                            securityKeyBean.getSecurityToken(), OssConfiguration.expire);
        } catch (Exception e) {
            log.error("Failed to obtain obs sts.", e);
        }
        return null;
    }
    ...
    public URL getObjectUrl(String bucket, String objectKey) {
        ...
        try {
            return new URL(this.extranetObsClient.createTemporarySignature(
                        new TemporarySignatureRequest(HttpMethodEnum.GET, bucket, objectKey,null, OssConfiguration.expire,new Date()))
                    .getSignedUrl());
